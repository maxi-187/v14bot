...
client.on("boostSubscriptionAdd", (boost) => {

    const guildID = "1169715609446141974";
    const channelID = "1200971116253040730";

    const targetGuild = client.guilds.cache.get(guildID);
    const boostChannel = targetGuild.channels.cache.get(channelID);

    boostChannel.send({
        embeds: [
          new EmbedBuilder()
            .setTitle(`**Neuer Booster!**`)
            .setDescription(
              `<@${boost.user.id}> hat den Server geboostet! 🚀`
            )
            .setColor('Red')
            .setFooter({ text: "© RG System" })
            .setTimestamp(),
