  ...
  callback: async (client, interaction) => {
    ...
    if (msDuration < 5000 || msDuration > 2.419e9) {
      await interaction.editReply(
        "Timeout Zeitraum kann nicht unter 5 Sekunden sein oder über 28 Tage!"
      );
      return;
    }
    ...
    try {
      ...
      if (targetUser.isCommunicationDisabled()) {
        ...
        await interaction.editReply(
          `${targetUser}'s timeout wurde verlängert auf ${prettyMs(msDuration, {
            verbose: true,
          })}\nGrund: ${reason}`
        );
        return;
      ...
      await targetUser.timeout(msDuration, reason);
      await interaction.editReply(
        `${targetUser} wurde getimeoutet für ${prettyMs(msDuration, {
          verbose: true,
        })}.\nGrund: ${reason}`
      );
    } catch (error) {
