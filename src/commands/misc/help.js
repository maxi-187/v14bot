...
module.exports = {
  ...
  callback: async (client, interaction) => {
    ...
    const ecom = new EmbedBuilder()
      .setColor("Purple")
      .setDescription(
        "`/level`: Zeigt dein Aktuelles Level oder von einem anderen User!\n`/leaderboard`: Zeigt die Top 10 mit den Höchsten Leveln\n`Es ist mehr in Arbeit!`\n"
      )
      .setTimestamp()
      .setFooter({ text: "© RG System" });
    ...
    collect.on("collect", async (i) => {
      if (i.customId === "menu") {
        ...
        if (
          value === "menu3" &&
          i.member.permissions.has(PermissionFlagsBits.ManageMessages)
        ) {
          ...
          await i.reply({
            embeds: [
              new EmbedBuilder()
                .setDescription(
                  "Du hast keine Rechte Dir das anzeigen zu lassen!"
                )
                .setColor("Red")
                .setFooter({ text: "© RG System" })
                .setTimestamp(),
        ...
        if (
          value === "menu4" &&
          i.member.permissions.has(PermissionFlagsBits.BanMembers)
        ) {
      ...
      if (i.customId === "economy") {
        ...
        await i.update({
          embeds: [ecom],
          components: [hilfemenü, allgemeinebefehle],
        });
      }
