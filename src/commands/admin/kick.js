...
module.exports = {
  ...
  callback: async (client, interaction) => {
    let role = interaction.guild.roles.cache.find(
      (r) => r.id == "1169715609878138997"
    ); // Staff
    let role1 = interaction.guild.roles.cache.find(
      (r) => r.id == "1169715609878138995"
    ); // Serverteam
      ...
      .send({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(
              `<@${interaction.user.id}> Hat Erfolgreich das Teammitglied ${benutzer} aus dem Team gekickt!`
            )
            .setColor(`#ED4245`)
            .setFooter({ text: "© RG System" })
