...
module.exports = {
    ...
    callback: async (client, interaction) => {
        ...
        if (!feedbackValue) {
            await interaction.reply({ content: 'Bitte gebe dein Feedback an!', ephemeral: true });
            return;
        }
        ...
        if (!serverRole || !targetUser.roles.cache.has(serverRole.id)) {
            await interaction.reply({ content: `Du kannst nur Mitglieder mit der ${serverRole} Rolle pingen!`, ephemeral: true });
            return;
        }

        const embed = new EmbedBuilder()
            .setTitle(`**Neues Feedback!**`)
            .setDescription(`<@${interaction.user.id}> **Schreibt Feedback über ${targetUser}**\n\n__**Feedback**__:\n${feedbackValue}`)
            .setColor('#ED4245')
        ...
        return interaction.reply({ embeds: [new EmbedBuilder().setDescription('Erfolgreich dein Feedback abgeschickt!').setColor('#57F287')], ephemeral: true });
    }
};
